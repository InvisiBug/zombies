...
void Fire::setPixelHeatColor(int pixel, uint8_t temperature, bool direction) {
  uint8_t heatLevel = round((temperature / 255.0) * 191);  // Scale 'heat' down from 0-255 to 0-191
  // uint8_t heatLevel = map(temperature, 0, 255, 0, 191); // Scale 'heat' down from 0-255 to 0-191

  // calculate ramp up from
  uint8_t heatRamp = heatLevel & 0x3F;  // 0..63
...
void Fire::setPixel(bool direction, int pixel, uint8_t red, uint8_t green, uint8_t blue) {
  if (!direction) {
